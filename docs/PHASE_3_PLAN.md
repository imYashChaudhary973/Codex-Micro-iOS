...
| **3.5** | `feat/reasoning-protocol` | Reasoning depth on the turn policy, Mac-clamped, plus a **live Codex probe** proving `turn/start` and `turn/steer` accept it. | Live probe passes. Written-from-documentation parameters are not trusted — two were wrong. |
| **3.6** | `feat/reasoning-dial` | The rotary control, adjustable mid-turn, showing the clamped effective level. | Turning past the grant ceiling clamps visibly rather than silently. |
