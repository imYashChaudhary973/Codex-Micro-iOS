...
7. **Strong single-use pairing:** The bootstrap secret has exactly 256 CSPRNG bits and is compared in constant time. Atomic `available → claimed` occurs before signature processing; a completed claim consumes the secret regardless of pairing success. Expired, losing concurrent, cancelled, reused, endpoint/mode/version-mismatched, signature-invalid, or incompletely confirmed attempts store no grant. The transcript-bound SAS has exactly 66 uniformly derived bits from a fixed versioned word list, and each endpoint requires its own local confirmation.
...
| **2.5 — `feat/crypto-pairing-state-machine`** | P0 | Transport-independent pairing state machine: normalized direct-LAN endpoint, five-minute session, constant-time bootstrap secret with exactly 256 CSPRNG bits, atomic claim-before-verification and consume-after-claim semantics, nonces, mutual transcript signatures, fixed/versioned SAS with exactly 66 derived bits, dual local confirmation, cancellation, and explicit initial grant (`observe` plus empty project scope by default). | Injectable clock/random/store; entropy/vector checks; expiry, rate-limited concurrent claim, reuse, cancellation, endpoint/mode mismatch, downgrade, signature/SAS mismatch, and one-sided confirmation tests; failed pairing stores no grant. |
...
| **2.12 — `feat/network-start-thread`** | P1 conditional — **closed by recorded deferral (2026-08-03); see [status](PHASE_2_STATUS.md)** | Product-decision checkpoint: if v1 allows new threads, add `startThread` behind an explicit Mac feature toggle, `startThread` capability, opaque allowlisted project ID, forced mobile profile, and no phone-supplied path/sandbox/policy. Otherwise move this entire PR to a later phase. | User decision recorded; feature off by default; opaque project resolution, policy ceiling, replay, and crash-ambiguity tests. If deferred by recorded decision, this step is not required for P0 completion, provisional completion, or Phase 2 acceptance. |
...
- **One plan step per PR.** Use the branch names above or the same scope/purpose convention. The Step 2.1 docs PR contains no checked-in spike code; the required spike merged as the separately reviewed `feat/spike-phase-2-transport` evidence PRs (#8/#9/#10). "Stacked" here means the evidence PRs branched from and merged to `origin/main` in evidence-first order **before** the docs step that cites them — not branches taken off the docs branch. Never combine protocol types with cryptography, cryptography with secure storage, pairing with the WSS listener, or the command gateway with all powerful executors.
...
- `startThread` — the Step 2.12 decision was recorded on 2026-08-03 as keeping v1 read/respond/control-only, so this is deferred to a later phase.
...
- **Transport layering:** WSS plus transcript-bound pinning and mutual identity signatures; post-handshake application payloads remain AEAD-sealed. The accepted stack (ADR §4) is a NIO Transport Services listener over Network.framework TLS with a Secure Enclave `SecIdentity`, NIOHTTP1/NIOWebSocket exact upgrade control, swift-certificates content-neutral certificates, and an SPKI-pinned `URLSessionWebSocketTask` client; TLS is 1.3-only with `AES_128_GCM_SHA256` and tickets/resumption/0-RTT/False Start/fallback/fast-open/peer-to-peer/multipath disabled (ADR §5).
- **Wire encoding:** the application schema is strict Codable JSON inside AEAD-sealed frames, and every sealed frame is carried as a WebSocket **binary** message — text frames are rejected (ADR §9). "No binary" refers only to the application schema: no protobuf and no compression until profiling proves a need.
...
- **Pairing:** five-minute, single-use 256-bit bootstrap secret; normalized direct-LAN endpoint; versioned SAS with exactly 66 derived bits; dual local confirmation; observe with empty project scope by default.
...
- **Dependencies:** resolved by the accepted transport ADR after the merged evidence spike proved the non-exportable Secure Enclave lifecycle. Exact pins: `swift-nio` 2.101.3, `swift-nio-transport-services` 1.28.0, `swift-certificates` 1.19.4, with authorized transitives `swift-atomics` 1.3.1, `swift-collections` 1.6.0, `swift-system` 1.7.5, `swift-crypto` 4.5.1, `swift-asn1` 1.7.1 (Swift tools 6.2+). NIOSSL is rejected (no public server ticket/resumption control under our policy); pure Network.framework WebSockets and custom framing are rejected (ADR §3). Production adoption in Steps 2.4a/2.7 uses these exact pins; any version change re-proves the affected feasibility rows (ADR §4).
...
Step 2.1 is complete: the threat model and transport ADR are accepted, and the evidence spike merged first as PRs #8/#9/#10 from `origin/main`. Begin **Step 2.2 only**: strict wire contracts and the journal epoch in `CompanionProtocol`, on `feat/protocol-secure-session-contracts` branched from the freshly merged `main`. Step 2.2 adds no cryptography target, no dependency, and no production listener, Bonjour advertisement, pairing endpoint, or network command path.
